 ...
 *
 * Usage: npx tsx scripts/run-first-cohort.ts
 * Or:    pnpm --filter @agentos/benchmarks exec tsx src/cli/run-cohort.ts
 */

import { createDefaultBenchmarkSuite, BenchmarkCategory } from '@agentos/benchmarks';
...
async function main(): Promise<void> {
  ...
  for (const c of criteria) {
    ...
    console.log(`    ${status}  ${c.label.padEnd(35)} ${c.value}`);
  }
  console.log('');
  console.log('═══════════════════════════════════════════════════════════');
